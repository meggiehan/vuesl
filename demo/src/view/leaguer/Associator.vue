    ...
    methods: {
      ...
      create (name) {
        this.types = [
          ...
          {name: 'save', text: '保存', url: 'associator_insert'}
        ]
        console.log(name)
        this.resetsingle()
      ...
      edit (idx) {
        this.types = [
          {name: 'quit', text: '退出', url: ''},
          {name: 'save', text: '保存', url: 'associator_update'}
        ]
...
//          api.post({JSON: JSON.stringify(updata)}, 'associator_delete').then((item) => {
//            this.getdata()
//          })
          api.mockDel({JSON: JSON.stringify(updata)}, 'associator').then((item) => {
            this.getdata()
          })
        })
      }
