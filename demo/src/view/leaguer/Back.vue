<template>
  <div class="wrap">
    ...
    <div class="role main">
      ...
      <div class="option">
        <button class="btn" @click="create('panel')">返积分</button>
      </div>
    ...
    methods: {
      ...
      create (name) {
        this.types = [
          ...
          {name: 'save', text: '保存', url: 'back_insert'}
        ]
        console.log(name)
        this.resetsingle()
      ...
      edit (idx) {
        this.types = [
          {name: 'quit', text: '退出', url: ''},
          {name: 'save', text: '保存', url: 'back_update'}
        ]
...
//          api.post({JSON: JSON.stringify(updata)}, 'back_delete').then((item) => {
//            this.getdata()
//          })
          api.mockDel({JSON: JSON.stringify(updata)}, 'back').then((item) => {
            this.getdata()
          })
        })
      }
